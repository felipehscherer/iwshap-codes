...
def evaluate_features(X_train, X_test, y_train, y_test, feature_importance_df, log, initial_features_count=1):
    """
...
def main(safe_path, attack_path, log_path, newdata_reduced):
    ...
    with open(log_file, "w") as log:
        ...
        log.write(f"Baseline: F1-Score={f1}, Recall={recall}, Precision={precision}, Accuracy={accuracy}\n")
        ...
        log.write(f"Melhor Precision: {best_precision_1}\n")
        log.write(f"Melhor Accuracy: {best_accuracy_1}\n")
        ...
        log.write(f"Melhor Precision: {best_precision_2}\n")
        log.write(f"Melhor Accuracy: {best_accuracy_2}\n")
        log.write(f"Melhor rodada: {best_round_2}\n")
        
        # Opcional: Salvar dataset reduzido com as melhores features da segunda execução
        if newdata_reduced:
            save_best_features_dataset(df, best_features_2)
